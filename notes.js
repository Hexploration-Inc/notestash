...
function saveNote(note) {
  ...
  console.log(
    "Current cache state before save:",
    Object.keys(notesCache).length,
    "notes"
  );
  ...
  chrome.storage.local.get(["notes"], function (result) {
    ...
    console.log(
      ...
      Object.keys(existingNotes).length,
      "existing notes"
    );
    ...
    console.log(
      "About to save notes object with",
      Object.keys(existingNotes).length,
      "notes"
    );
      ...
      if (chrome.runtime.lastError) {
        ...
        chrome.storage.local.get(["notes"], function (verifyResult) {
          if (verifyResult.notes && verifyResult.notes[note.id]) {
            ...
            console.error(
              "Storage verification FAILED: Note not found in storage after save!"
            );
          }
        });
      }
...
function createNoteUI() {
  ...
  noteButton.innerHTML =
    '<svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M20 2H4C2.9 2 2 2.9 2 4V22L6 18H20C21.1 18 22 17.1 22 16V4C22 2.9 21.1 2 20 2M20 16H5.2L4 17.2V4H20V16Z"></path></svg> Add Note';
...
function addNoteToSelection() {
  ...
  // Check if this is likely a multi-line selection
  const isMultiLine = selectedText.includes("\n") || rect.height > 30; // Basic threshold for multi-line

  // For multi-line selections, try to place the indicator at the end of the last line
  let indicatorPosition;

  if (isMultiLine) {
    // Create a new range for the end point of the selection
    const endRange = document.createRange();
    endRange.setStart(range.endContainer, range.endOffset);
    endRange.setEnd(range.endContainer, range.endOffset);
    const endRect = endRange.getBoundingClientRect();

    // Store these coordinates for later use when creating the note indicator
    indicatorPosition = {
      left: endRect.right + window.scrollX,
      top: endRect.top + window.scrollY,
    };
    console.log(
      "Multi-line selection detected, using end position for indicator"
    );
  } else {
    // For single-line selections, use the right side of the selection
    indicatorPosition = {
  ...
  notePopup.dataset.selectionLeft = indicatorPosition.left;
  notePopup.dataset.selectionTop = indicatorPosition.top;
...
function createNoteIndicator(noteId, position) {
  ...
  indicator.innerHTML =
    '<svg viewBox="0 0 24 24" width="12" height="12"><path fill="currentColor" d="M20 2H4C2.9 2 2 2.9 2 4V22L6 18H20C21.1 18 22 17.1 22 16V4C22 2.9 21.1 2 20 2Z"></path></svg>';
  ...
  if (position) {
    // We already added window.scrollX/Y when storing the position in addNoteToSelection
      ...
      if (noteId) {
        ...
        if (removeNote(noteId)) {
          ...
          const indicator = document.querySelector(
            `.note-indicator[data-note-id="${noteId}"]`
          );
          if (indicator) {
            indicator.remove();
          }
...
function setupNoteEventListeners() {
  ...
  if (saveButton) {
    ...
    newSaveButton.onclick = function (event) {
      ...
      const noteId =
        editNoteId || `note-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
      ...
      const occurrenceIndex = parseInt(
        notePopup.dataset.occurrenceIndex || "1"
      );
      ...
      if (notePopup.dataset.selectionLeft && notePopup.dataset.selectionTop) {
        ...
        if (selection.rangeCount > 0) {
          ...
          console.log(
            "Note indicator position from current selection:",
            position
          );
        } else {
      ...
      setTimeout(() => {
        ...
        if (selectedText) {
          ...
          if (selection.rangeCount > 0) {
            ...
            console.log(
              "Note button displayed at position:",
              noteButton.style.top,
              noteButton.style.left
            );
          }
        } else {
